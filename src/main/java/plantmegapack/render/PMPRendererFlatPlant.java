...
public class PMPRendererFlatPlant implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (!(block instanceof PMPBlockPlant)) {
            ...
            this.renderFlatPlant(tessellator, iconMinU, iconMaxU, iconMinV, iconMaxV, world.getBlockMetadata(x, y, z));
            tessellator.addTranslation((float) (-x), (float) (-y), (float) (-z));
            return true;
        }
    }
    ...
    private void renderFlatPlant(Tessellator tessellator, double iconMinU, double iconMaxU, double iconMinV,
        ...
        double angle = (double) (metaData * 45 + 90);
        if (angle > 360.0) {
            angle -= 360.0;
        }
